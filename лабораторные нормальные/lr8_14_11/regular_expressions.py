...
class Input_system():
    ...
    def __init__(self) -> None:
        self.nik = input("Введите имя пользователя:")
        self.pas = input("Введите пароль:")
        pattern_password =r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%?&])[A-Za-z\d@$!%*?&]{8,}$"
        invalid_pass_text = \
        "Ваш пароль должен содержать не менее 8 символов, по крайней мере одну заглавную букву, строчную букву, цифру и специальный символ, чтобы быть безопасным."
        if (re.search(pattern_password, self.pas)):
            print("Данный пароль подходит🐱‍👤🐱‍🐉")
        else:
